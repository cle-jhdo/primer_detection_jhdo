...
class DecoderV2(nn.Module):
    def __init__(self, config):
        ...
        self.dec5_1 = CBR2d(in_channels=config.unet_n_filters[4] + config.dilation_n_filters[1], out_channels=config.unet_n_filters[3])
        ...
        # adding layers for getting global features
        self.dec_dilate = CBR2d(in_channels=config.unet_n_filters[4], out_channels=config.dilation_n_filters[0], dilation=config.dilation, padding='same')

        self.dec_1x1 = CBR2d(in_channels=config.dilation_n_filters[0], out_channels=config.dilation_n_filters[1], kernel_size=1, padding=0)


    def forward(self, x, encoder):
        encoder.eval()
        with torch.no_grad(): # Freeze encoder layers
            enc1_1 = encoder.enc1_1(x)
            enc1_2 = encoder.enc1_2(enc1_1)
            pool1 = encoder.pool1(enc1_2)

            enc2_1 = encoder.enc2_1(pool1)
            enc2_2 = encoder.enc2_2(enc2_1)
            pool2 = encoder.pool2(enc2_2)

            enc3_1 = encoder.enc3_1(pool2)
            enc3_2 = encoder.enc3_2(enc3_1)
            pool3 = encoder.pool3(enc3_2)

            enc4_1 = encoder.enc4_1(pool3)
            enc4_2 = encoder.enc4_2(enc4_1)
            pool4 = encoder.pool4(enc4_2)

            enc5_1 = encoder.enc5_1(pool4)
        
        dec_dilated = self.dec_dilate(enc5_1)
        dec_global = self.dec_1x1(dec_dilated)

        enc5_2 = torch.cat((enc5_1, dec_global), 1)
